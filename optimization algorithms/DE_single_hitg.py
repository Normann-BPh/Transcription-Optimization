...
with open('../dicts/SQP/SQP_HitG_50_single_alpha_dict.json','r') as f:
    alpha_pos_dict = json.load(f)
with open('../dicts/SQP/SQP_HitG_50_single_beta_dict.json','r') as f:
    beta_pos_dict = json.load(f)
with open('../dicts/SQP/SQP_HitG_50_single_error_dict.json','r') as f:
...
last_fm = list(error_pos_dict.items())[-1][-1][-1] + 9
last_fp = list(error_pos_dict.items())[-1][-1][0] + 9
...
class MyProblem(ElementwiseProblem):
    ...
    def __init__(self, **kwargs):
        ...
        super().__init__(n_var = n_var, 
                        n_obj = 1, 
                        n_eq_constr = len(mRNA_df.columns)*9+len(alpha_pos_dict)+len(beta_pos_dict), 
                        xl = boundsl, 
                        xu = boundsu)
    ...
    def DE_sin_con_RNA(self,x):
        ...
        for R in self.mRNA.columns:
            ...
            unit = np.array(self.mRNA[R]) - np.sum(unit,axis=0) - fp + fm
...
name = '_single_best1bin_{}_{}_pop{}_CR{}_WF{}_FRS_{}_{}'.format(n_threads,n_gen,pop,CR,WF,d1.strftime('%d-%m-%Y'),int(Time))
