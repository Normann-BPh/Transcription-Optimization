...
class MyProblem(ElementwiseProblem):
    ...
    def __init__(self, **kwargs):
        ...
        super().__init__(n_var = n_var, 
                        n_obj = len(mRNA_df.columns)*9, 
                        n_eq_constr = len(alpha_pos_dict)+len(beta_pos_dict), 
                        xl = boundsl, 
                        xu = boundsu)

    def _evaluate(self, x, out):
        out["F"] = np.abs(self.GA_sin(x))
    ...
    def GA_sin(self, x):
        ...
        for R in self.mRNA.columns:
            ...
            unit = np.array(self.mRNA[R]) - np.sum(unit,axis=0)
            F_res = np.concatenate((F_res,unit))
        return F_res
...
ftol_termination = RobustTermination(MultiObjectiveSpaceTermination(tol=0.005, n_skip = 1000), period=2)
...
name = 'single_{}_pop{}_off{}_gen{}_SBX{}_PM{}_FRS_{}_T{}'.format(n_threads,pop_size,n_off,num_gen,sbx,pm,d1.strftime('%d-%m-%Y'),int(Time))